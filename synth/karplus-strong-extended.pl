...
my $tau = $sec;   # time in seconds where waveform loses 60dB; arbitrary here
my $g = 10**(-6 / ($freq * $tau));
my $b0 = 0.5 * $g * $t;
my $b1 = 0.5 * $g * ($t + 1);   # I assumed (1/g)g(t+1) was a typo (?)
my $b2 = 0.5 * $g;
my $a = $t;
...
my $vel = 120;   # 0 - 127 (midi), arbitrary here
my $vmin = 0.01;   # should be a "small" value
my $v = $vmin**(1 - $vel / 128) - 1;
my $w = 0;

# 4. plucking-point filter: 0 < u <= 0.5; 0.5 = half-string point (flute-like)
my $u = 0.5;  # arbitrary
...
foreach my $n (0 .. $num_samples - 1) {
    ...
    print pack($packfmt, int($MAX_AMPLITUDE * V(P($y, $n))));
}
